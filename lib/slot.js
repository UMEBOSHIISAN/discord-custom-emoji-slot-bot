...
// --- 設定1〜6（日替わり） ---
const SETTING_MULTIPLIERS = [0.7, 0.85, 1.0, 1.2, 1.5, 2.0]; // 設定1〜6
const SETTING_HINTS = ['低挙動', '平凡', '普通', 'ちょい高', '高挙動', '激アツ'];

function getTodaySetting() {
  const d = new Date();
  const seed = d.getFullYear() * 10000 + (d.getMonth() + 1) * 100 + d.getDate();
  // 簡易ハッシュで日替わり（均等分布）
  const hash = ((seed * 2654435761) >>> 0) % 6;
  return hash; // 0〜5（設定1〜6）
}

function getSettingMultiplier(settingIndex) {
  return SETTING_MULTIPLIERS[settingIndex] || 1.0;
}

function getSettingHint(settingIndex) {
  return SETTING_HINTS[settingIndex] || '';
}

// --- 抽選 ---
function rollOutcome(cfg, consecutiveLosses, kakuhenActive, settingIndex) {
  ...
  const kakuhenMult = kakuhenActive ? (cfg.KAKUHEN_MULTIPLIER || 3) : 1;
  const settingMult = (cfg.enableSetting !== false && settingIndex != null)
    ? getSettingMultiplier(settingIndex) : 1;
  const jp = Math.min(cfg.JACKPOT_PROB * kakuhenMult * settingMult, 0.5);
  const r = Math.random();
...
async function animateSpin(message, pool, final, cfg, outcome, winInfo) {
  ...
  // --- 前兆演出 ---
  // JACKPOT確定時は高確率で前兆、ハズレ時はフェイク前兆が低確率
  const fakeZenchoProb = cfg.FAKE_ZENCHO_PROB || 0.03;
  const realZenchoProb = cfg.REAL_ZENCHO_PROB || 0.7;
  const doZencho = cfg.enableZencho !== false && (
    (outcome.result === 'jackpot' && Math.random() < realZenchoProb) ||
    (outcome.result !== 'jackpot' && Math.random() < fakeZenchoProb)
  );
  // 前兆ステップ: Phase A 中に1〜2回
  let zenchoSteps = [];
  if (doZencho && phB > 3) {
    const count = 1 + Math.floor(Math.random() * 2); // 1〜2回
    const used = new Set([fakeStep, reverseStep]);
    for (let i = 0; i < count; i++) {
      let s;
      do { s = 2 + Math.floor(Math.random() * (phB - 2)); } while (used.has(s));
      used.add(s);
      zenchoSteps.push(s);
    }
    zenchoSteps.sort((a, b) => a - b);
  }
  ...
  for (let step = 2; step <= totalN; step++) {
    ...
    if (isStop) {
      ...
      } else if (zenchoSteps.includes(step)) {
        // 前兆演出: 色チラッ + テキスト変化
        const zenchoMsgs = ['ん…？', 'ざわ…', '！？', 'なんか来た…？'];
        title = `${kakuhenPrefix}🎰 ${pick(zenchoMsgs)} [${step}/${totalN}]`;
        desc = `${eStr(L)} ${eStr(M)} ${eStr(R)}`;
        color = 0xDD2222; // 赤チラッ
      } else {
      ...
      if (isReach) {
        const reachMsgs = ['リーチ？', 'リーチ！？', 'メモリーチ！？', 'メモリーチ！？', 'まさか…リーチ！？', 'ガチリーチ！？'];
        title += ` ${pick(reachMsgs)}`;
      }
      // レインボー演出: Embed 色を虹色に
    ...
    if (isStop) {
      ...
      const settingHint = (cfg.enableSetting !== false && winInfo.settingIndex != null)
        ? ` | 挙動: ${getSettingHint(winInfo.settingIndex)}` : '';
      const ptsTxt = winInfo.points != null ? ` | SP: ${winInfo.points}` : '';
      em.setFooter({ text: `勝率: ${rate}% (${winInfo.wins}/${winInfo.spins})${ptsTxt}${settingHint}` });
    }
    await msg.edit({ embeds: [em] }).catch(() => {});
  }
...
module.exports = {
  ...
  animateSpin, maybeSendGifOnce, fireParticles,
  getTodaySetting, getSettingHint, getSettingMultiplier,
  eStr, pick,
};
